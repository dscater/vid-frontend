  ...
  const eliminarCategoria = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let respuesta = await api.post("/admin/categorias/" + item.id, {
          _method: "DELETE",
        });
          ...
          <div class="col-md-4">
            <button
              v-if="
                ...
                authStore?.user?.permisos.includes('categorias.create')
              "
              type="button"
        ...
        <div class="row">
          <div class="col-12">
            <MiTable
              ...
              :cols="headers"
              :api="true"
              :url="apiUrl + '/admin/categorias/paginado'"
              :numPages="5"
              :perPage="20"
            ...
            >
              <template #accion="{ item }">
                <template
                  v-if="
                    authStore?.user?.permisos == '*' ||
                    authStore?.user?.permisos.includes('categorias.edit')
                  "
                >
                  <el-tooltip
                    class="box-item"
                ...
                <template
                  v-if="
                    authStore?.user?.permisos == '*' ||
                    authStore?.user?.permisos.includes('categorias.destroy')
                  "
                >
                  <el-tooltip
                    class="box-item"

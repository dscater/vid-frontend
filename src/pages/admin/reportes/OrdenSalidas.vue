<script setup>
  ...
  const generarReporte = () => {
    ...
    api
      .post("admin/reportes/orden_salidas", form, {
        responseType: "blob",
      })
      .then((response) => {
        form.errors = [];
        if (form.tipo == "pdf") {
          ...
          fileLink.setAttribute("download", "orden_salidas.xlsx");
...
<template>
  <Content>
    <template #header>
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0">Reportes > Órdendes de Salida</h1>
        </div>
        <!-- /.col -->
        <div class="col-sm-6">

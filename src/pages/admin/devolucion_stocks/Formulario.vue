<script setup>
  ...
  const { oDevolucionStock, limpiarDevolucionStock } = useDevolucionStocks();
  const accion_form = ref(props.accion_formulario);
  const muestra_form = ref(props.muestra_formulario);
  const enviando = ref(false);
  let form = reactive(oDevolucionStock.value);
  ...
  const tituloDialog = computed(() => {
    return accion_form.value == 0
      ...
      : `<i class="fa fa-edit"></i> Editar Devolución de Stock`;
  });
  ...
  const enviarFormulario = () => {
    ...
    let url =
      accion_form.value == 0
        ? "/admin/devolucion_stocks"
        : "/admin/devolucion_stocks/" + form.id;
  ...
  const agregarProducto = () => {
    ...
    api
      ...
      .then((response) => {
        ...
        form.devolucion_stock_detalles.push({
          id: 0,
  ...
  const calcularSubtotal = (e, index) => {
    ...
    if (!value || value.trim() == "") {
      form.devolucion_stock_detalles[index].subtotal =
        form.devolucion_stock_detalles[index].costo;
    }
    form.devolucion_stock_detalles[index].subtotal =
      parseFloat(value) *
  ...
  const calcularTotal = () => {
    if (form.devolucion_stock_detalles.length == 0) {
      form.total = 0;
      form.cantidad_total = 0;
      return;
    }
    let total = 0;
    total = form.devolucion_stock_detalles.reduce((acum, item) => {
        ...
        <form @submit.prevent="enviarFormulario()">
          ...
          <div class="row">
            ...
            <div class="col-12 overflow-auto">
              ...
              <ul
                v-if="form.errors?.devolucion_stock_detalles"
                class="d-block text-danger list-unstyled"
              >
                <li class="parsley-required">
                  {{ form.errors?.devolucion_stock_detalles[0] }}
                </li>
              </ul>
            </div>
          </div>

    ...
    >
      <template v-for="(item, index) in archivos_existentes">
        <div class="archivo">
          ...
          <span
            class="check"
            :class="[item.id != 0 ? 'text-success' : 'text-grey-darken-1']"
            ><i
              class="mdi"
              :class="[item.id != 0 ? 'mdi-check-circle' : 'mdi-upload-circle']"
            ></i
          ></span>
          <div class="thumbail">
  ...
  export default {
    ...
    methods: {
      ...
      handleFiles(eventOrFiles) {
        ...
        let total_cargados =
          parseInt(files.length) + parseInt(this.archivos_existentes.length);

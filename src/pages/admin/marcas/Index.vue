<script setup>
  ...
  const { setMarca, limpiarMarca } = useMarcas();
  ...
  const eliminarMarca = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        let respuesta = await api.post("/admin/marcas/" + item.id, {
          _method: "DELETE",
        });
          ...
          <div class="col-md-4">
            <button
              v-if="
                ...
                authStore?.user?.permisos.includes('marcas.create')
              "
              type="button"
            ...
            >
              <template #accion="{ item }">
                <template
                  v-if="
                    authStore?.user?.permisos == '*' ||
                    authStore?.user?.permisos.includes('marcas.edit')
                  "
                >
                  <el-tooltip
                    class="box-item"
                ...
                <template
                  v-if="
                    authStore?.user?.permisos == '*' ||
                    authStore?.user?.permisos.includes('marcas.destroy')
                  "
                >
                  <el-tooltip
                    class="box-item"
                    ...
                    placement="left-start"
                  >
                    <button class="btn btn-danger" @click="eliminarMarca(item)">
